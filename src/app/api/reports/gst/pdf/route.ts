...
import { generateGstRegisterPdf } from "@/lib/reports/gst-register-pdf";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `GST_Expense_Register_${timestamp}.pdf`;
    ...
    console.error("GST PDF export error:", err);
    return NextResponse.json(
      { error: err.message || "Failed to generate GST PDF report." },
      { status: 500 }
    );
  }
}
